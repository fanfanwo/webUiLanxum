...
SHEET_NAME = '%sheet_name'  # store the path to JSON file
ROW = '%row'  # 行数
COL = '%col'  # 列数
ROW_OFFSET = "%row_offset"  # 行数偏移量
COL_OFFSET = "%col_offset"  # 列数偏移量
...
def excel_data(value, sheet_name="Sheet1", row=None, col=None, row_offset=None):
    """更改源码，将Excel的读取方式写到源码里面来"""

    def wrapper(func):
        setattr(func, FILE_ATTR, value)
        setattr(func, SHEET_NAME, sheet_name)
        if row is not None:
            setattr(func, ROW, row)
        if col is not None:
            setattr(func, COL, col)
        if row_offset is not None:
            setattr(func, ROW_OFFSET, row_offset)
...
def process_file_data(cls, name, func, file_attr):
    ...
    # print(cls)  # <class '数据驱动2.Test'>
    data_file_path = os.path.join(os.path.dirname(cls_path), file_attr)

    # print(data_file_path)  # /Volumes/huace/Web自动化/class11/课程脚本/pythonProject1/file.yaml
    ...
    if not os.path.exists(data_file_path):  # 判断文件是否存在
    ...
    elif _is_excel_file and not _have_xlrd:
        test_name = mk_test_name(name, "error")
        test_docstring = """Error!"""
        add_test(
            cls,
    ...
    with codecs.open(data_file_path, 'r', 'utf-8') as f:
        ...
        if _is_yaml_file:
            ...
            1,按行读取  需要行数
            2,按列读取  需要列数
            3,读取数据并组装成一个可迭代对象 （list[dict]）
            """
            workbook_object = xlrd.open_workbook(data_file_path)
            sheet_name = getattr(func, SHEET_NAME)
            sheet_object = workbook_object.sheet_by_name(sheet_name)
            if hasattr(func, ROW):
                row = getattr(func, ROW)
                data = sheet_object.row_values(row)

            elif hasattr(func, COL):
                col = getattr(func, COL)
                data = sheet_object.col_values(col)

            elif hasattr(func, ROW_OFFSET):  # 偏移量的数据用元组最好()
                row_offset = getattr(func, ROW_OFFSET)
                data = list()

                start_row, end_row = row_offset  # 是一个元组
                keys = sheet_object.row_values(start_row)
                for i in range(1, end_row + 1):
                    row_value = sheet_object.row_values(i)
                    data.append(dict(zip(keys, row_value)))
                # print(data)
        else:
            data = json.load(f)
